...
def plot_cartmap(lvc_healpix_file, levels=[0.5, 0.95], angsize=3., tile_ra=None, tile_dec=None, targ_ra=None, targ_dec=None):
    ...
    if tile_ra is not None and tile_dec is not None:
        for _ra_c, _dec_c in zip(tile_ra, tile_dec):
            circ = plt.Circle((_ra_c, _dec_c), radius=1.6, fc='None', ec='b', ls=':', lw=2,alpha=0.5)
            ax.add_artist(circ)
    ...
    if targ_ra is not None and targ_dec is not None:
        ax.plot(targ_ra, targ_dec, 'k.', alpha=0.1,markersize=2.5)
    for _ra_c, _dec_c in zip([218.9,217.5],[36.6,35.8]):
        circ1=plt.Circle((_ra_c,_dec_c),radius=1.6,fc='None',ec='r',ls='-.',lw=2,alpha=0.7)
        ax.add_artist(circ1)
    _h.append(circ1)
    _l.append('DESI-GW')
    ...
    cb = fig.colorbar(img, location='left', shrink=0.95,
